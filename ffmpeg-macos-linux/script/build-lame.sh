...
# lame 3.100 has no working --disable-frontend configure flag; the frontend
# subdirectory uses langinfo.h which doesn't exist on Windows. Remove it from
# the generated Makefile's SUBDIRS directly — we only need libmp3lame.
sed -i.original 's/ frontend//' Makefile
checkStatus $? "remove frontend from Makefile SUBDIRS failed"
