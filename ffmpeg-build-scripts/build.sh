...
for arg in "$@"; do
    ...
    if [ $KEY = "-FFMPEG_ONLY" ]; then
        FFMPEG_ONLY=$VALUE
        echo "build only ffmpeg binary $VALUE"
    fi
    if [ $KEY = "-FFMPEG_EXTRA_VERSION" ]; then
        FFMPEG_EXTRA_VERSION=$VALUE
        echo "use ffmpeg extra version $VALUE"
    fi
...
if [ "$FFMPEG_ONLY" = "YES" ]; then
    SKIP_SDL="YES"
    FFMPEG_ONLY_FLAGS="--disable-ffplay --disable-ffprobe"
fi

# CLANGARM64 has no 'gcc' or 'cc' alias; autoconf configure scripts default
# to searching for those names and fail. Export CC/CXX once here so every
# child script (autoconf, meson, cmake) inherits the correct compiler.
...
if isMsys; then
    ...
    echoSection "neutralize Windows runtime import libraries"
    neutralizeWindowsToolchainImportLibs > "$LOG_DIR/neutralize-windows-runtime-import-libs.log" 2>&1
    checkStatus $? "neutralize Windows runtime import libraries"
...
# prepare build
FFMPEG_LIB_FLAGS="$FFMPEG_ONLY_FLAGS"
REQUIRES_GPL="NO"
...
# Build custom libiconv only on Windows. Linux has glibc iconv and the
# disabled shared libiconv path caused problems for zvbi on other platforms.
if isMsys; then
    START_TIME=$(currentTimeInSeconds)
...
if [ $SKIP_SDL = "NO" ]; then
    ...
    echoDurationInSections $START_TIME
    echo "NO" > "$LOG_DIR/skip-sdl"
else
    echoSection "skip SDL"
    echo "YES" > "$LOG_DIR/skip-sdl"
fi
...
if [ "$ENABLE_MACOS_STATIC_LINK_HARDENING" = "YES" ]; then
    pruneToolDynamicLibraries "$TOOL_DIR"
    checkStatus $? "prune tool dynamic libraries"
    pruneHomebrewDynamicLibraries
    checkStatus $? "prune Homebrew dynamic libraries"
    validateToolLinkageMetadata "$TOOL_DIR"
    checkStatus $? "validate tool linkage metadata"
else
    echo "skip macOS static link hardening"
fi
$SCRIPT_DIR/build-ffmpeg.sh "$SCRIPT_DIR" "$SOURCE_DIR" "$TOOL_DIR" "$OUT_DIR" "$CPUS" "$FFMPEG_SNAPSHOT" "$FFMPEG_LIB_FLAGS" "$FFMPEG_EXTRA_VERSION" > "$LOG_DIR/build-ffmpeg.log" 2>&1
