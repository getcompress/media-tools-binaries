...
# cmake 4.x removed OLD-behaviour support for CMP0025 (AppleClang id) and
# CMP0054 (if() quoting). x265 4.x CMakeLists.txt still sets both to OLD,
# which is now a hard error. Remove those lines before any cmake invocation.
sed -i.original \
    -e '/cmake_policy[[:space:]]*(SET CMP0025/d' \
    -e '/cmake_policy[[:space:]]*(SET CMP0054/d' \
    source/CMakeLists.txt
checkStatus $? "patch x265 CMakeLists.txt for cmake 4.x failed"
...
if [ $SKIP_X265_MULTIBIT = "NO" ]; then
    ...
    cd 10bit/
    checkStatus $? "change directory failed"
    cmake $(cmakeTargetArgs) $(cmakePolicyCompatArgs) -DCMAKE_INSTALL_PREFIX:PATH=$TOOL_DIR -DENABLE_SHARED=NO -DENABLE_CLI=OFF -DEXPORT_C_API=OFF -DHIGH_BIT_DEPTH=ON ../source
    ...
    cd 12bit/
    checkStatus $? "change directory failed"
    cmake $(cmakeTargetArgs) $(cmakePolicyCompatArgs) -DCMAKE_INSTALL_PREFIX:PATH=$TOOL_DIR -DENABLE_SHARED=NO -DENABLE_CLI=OFF -DEXPORT_C_API=OFF -DHIGH_BIT_DEPTH=ON -DMAIN12=ON ../source
    ...
    cmake $(cmakeTargetArgs) $(cmakePolicyCompatArgs) -DCMAKE_INSTALL_PREFIX:PATH=$TOOL_DIR -DENABLE_SHARED=NO -DENABLE_CLI=OFF \
    ...
    if [ "$(uname)" = "Linux" ] || isMsys; then
        AR_CMD="$(command -v llvm-ar 2>/dev/null || command -v ar 2>/dev/null || echo ar)"
        "$AR_CMD" -M <<EOF
CREATE libx265.a
...
EOF
    ...
    # prepare build
    cmake $(cmakeTargetArgs) $(cmakePolicyCompatArgs) -DCMAKE_INSTALL_PREFIX:PATH=$TOOL_DIR -DENABLE_SHARED=NO -DENABLE_CLI=OFF source
